...
The system SHALL select at most four named `:free` persona models from the operator configuration, exclude the dynamic `openrouter/free` router, and use the next model only after a provider, shape, or shared output-gate rejection. It MUST NOT use a paid fallback, modify the autonomous-learning allowlist, or make an unbounded number of model calls.
