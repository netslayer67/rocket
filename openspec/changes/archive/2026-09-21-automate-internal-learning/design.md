...
- Autonomous chat does not use OpenRouter's dynamic `openrouter/free` router because it can select an unreviewed provider for approved internal evidence. It instead permits only three currently free, structured-output candidates in configurable order: NVIDIA Nemotron 3 Super, Nex N2.5 Mini, and Nex N2.5 Pro. Their reasoning controls differ, so the first uses low effort and the Nex fallbacks use none. Poolside, domain-specialized, multimodal, and expiring preview models remain excluded unless their privacy and contract fit are explicitly reviewed.
- Production's first response used its entire 1,600-token allowance and returned invalid JSON. Policy revision 2 requests low reasoning effort and allows 3,200 synthesis / 1,200 review tokens; length-truncated completions trigger bounded free fallback rather than success telemetry. Policy revision 3 changes the trusted model allowlist, so it legitimately retries the unchanged evidence once under the normal per-batch daily bound. Invalid JSON is a retriable operational failure, not a permanent quality rejection. Daily limits remain unchanged and resolved model IDs are recorded when available.
