...
export function ThreadsConnection({ status, busy, onDisconnect }: { status: ThreadsStatus; busy: boolean; onDisconnect: () => Promise<boolean> }) {
  ...
  return <details className="mt-8 border-y border-slate-800 py-4">
    <summary className="cursor-pointer text-base font-semibold text-white">Koneksi Threads <span className="ml-2 text-sm font-normal text-slate-400">(opsional, untuk publish manual)</span></summary>
