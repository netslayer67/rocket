...
export function AnalyticsPanel({ summary, insights, narratives, busy, onCapture, onPromote }: { summary: AnalyticsSummary; insights: AnalyticsInsight[]; narratives: Narrative[]; busy: boolean; onCapture: (input: AnalyticsInput) => Promise<boolean>; onPromote: (narrativeId: string, lessonType: 'positive' | 'negative') => Promise<boolean> }) {
  ...
  return <details className="mt-8 border-y border-slate-800 py-4">
    <summary className="cursor-pointer text-base font-semibold text-white">Sinyal hasil <span className="ml-2 text-sm font-normal text-slate-400">(opsional)</span></summary>
    <p className="mt-3 text-sm leading-6 text-slate-400">Catat angka yang kamu lihat; Rocket menghitung CTR dan engagement.</p>
    ...
    <p className="mt-4 text-xs leading-5 text-slate-400">Feedback yang kamu izinkan dipelajari otomatis. Metrik baru menjadi kandidat untuk ditinjau; kandidat tetap perlu persetujuan sebelum menjadi DNA.</p>
    ...
    <div className="mt-4 flex flex-wrap gap-2"><button className="button-secondary" type="button" onClick={() => setOpen(!open)}>{open ? 'Tutup input metrik' : 'Catat metrik manual'}</button><span className="self-center text-xs text-slate-500">Sumber: {summary.source}</span></div>
