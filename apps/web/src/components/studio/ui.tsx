...
export function SectionCard({ title, description, children, wide = false }: { title: string; description: string; children: ReactNode; wide?: boolean }) {
  return (
    <section className={`section-card ${wide ? 'lg:col-span-2' : ''}`}>
      <div className="mb-5"><h2 className="text-base font-semibold text-white">{title}</h2><p className="mt-1 text-sm text-slate-400">{description}</p></div>
      {children}
    </section>
  );
}
...
export function StatusBadge({ status }: { status: 'draft' | 'approved' }) {
  ...
  return <span className={`rounded-full px-2.5 py-1 text-xs font-medium ring-1 ${color}`}>{status === 'approved' ? 'Approved' : 'Draft'}</span>;
}
...
export function VectorBadge({ status }: { status: 'pending' | 'ready' }) {
  ...
  return <span className={`rounded-full px-2.5 py-1 text-xs font-medium ring-1 ${color}`}>{status === 'ready' ? 'Semantic ready' : 'Index pending'}</span>;
}
