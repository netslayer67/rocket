...
export function KnowledgeLibrary({ knowledge, busy, onReindex }: { knowledge: Knowledge[]; busy: boolean; onReindex: () => Promise<boolean> }) {
  return (
    <section className="mt-8" aria-labelledby="library-title">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><p className="eyebrow">Knowledge engine · V2</p><h2 id="library-title" className="mt-1 text-xl font-bold text-white">Knowledge Library</h2><p className="mt-1 text-sm text-slate-400">Pola tersimpan sebagai metadata; sumber mentah tidak masuk database.</p></div><button className="button-secondary" disabled={busy || knowledge.length === 0} onClick={() => void onReindex()}>Reindex semantic search</button></div>
