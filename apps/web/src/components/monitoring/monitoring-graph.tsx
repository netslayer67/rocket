...
const signalNodes = ['Jobs', 'Feedback', 'Metrics'];
const stageNames: Record<Stage, string> = { signals: 'Signals', agents: 'Agents', models: 'Models', memory: 'Memory' };
const stageX: Record<Stage, number> = { signals: 8, agents: 34, models: 62, memory: 88 };

export function MonitoringGraph({ events }: { events: MonitoringEvent[] }) {
  ...
  const paths = makePaths(nodes);
  const activePaths = paths.filter((path) => active.has(path.from) && active.has(path.to));
  const state = recent.length ? `${recent.length} event terbaru menggerakkan flow` : events.length ? 'Idle · histori tetap tersimpan' : 'Idle · belum ada event 24 jam';
  return (
    <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950" aria-label="Peta aliran neural workflow berdasarkan event tersimpan">
      <div className="min-w-[980px] p-4 sm:p-6">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500">
          <span className="font-medium uppercase tracking-[0.16em] text-slate-400">Persisted workflow</span>
        ...
        <div className="relative h-[430px] overflow-hidden rounded-xl border border-slate-800 bg-slate-950">
          ...
          {(Object.keys(stageNames) as Stage[]).map((stage) => <div key={stage} className="absolute top-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500" style={{ left: `${stageX[stage] - 6}%` }}>{stageNames[stage]}</div>)}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-slate-500" aria-label="Legenda status workflow">
          <Legend color="bg-cyan-300" label="recent event" /><Legend color="bg-slate-600" label="history / idle" /><span className="ml-auto">SSE bounded stream · timeline adalah fallback lengkap</span>
        </div>
      </div>
    </div>
  );
...
function makePaths(nodes: Node[]): Path[] {
  ...
  return [
    ...
    ...agents.flatMap((agent, index) => [route(agent.id, models[index % models.length].id), route(agent.id, models[(index + 1) % models.length].id)]),
    ...models.flatMap((model, index) => [route(model.id, memory[index % memory.length].id), route(model.id, memory[(index + 1) % memory.length].id)]),
  ];
}
...
function activeNodes(recent: MonitoringEvent[], nodes: Node[]) {
  ...
  recent.forEach((event) => {
    ...
    if (event.kind === 'knowledge' || event.kind === 'learning') active.add('memory:qdrant');
...
function GraphNode({ node, active }: { node: Node; active: boolean }) {
  return <div className={`absolute z-10 w-44 -translate-x-1/2 -translate-y-1/2 rounded-lg border px-3 py-2.5 transition-colors ${active ? 'neural-node-active border-cyan-300/70 bg-cyan-950 text-cyan-50' : 'border-slate-800 bg-slate-900 text-slate-300'}`} style={{ left: `${node.x}%`, top: `${node.y}%` }} role="listitem" aria-label={`${node.label}: ${active ? 'active' : 'idle'}`}>
    ...
    {node.note && <span className="ml-4 mt-1 block truncate text-[10px] uppercase tracking-[0.12em] text-slate-500">{node.note}</span>}
