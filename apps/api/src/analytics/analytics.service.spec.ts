...
describe('AnalyticsService', () => {
  ...
  it('promotes an explicitly approved positive candidate once', async () => {
    ...
    const narrative = { _id: 'n1', personaId: 'active', title: 'Sudut kecil', topic: 'observasi', linkPlacement: 'ending' };
    ...
    expect(createLesson).toHaveBeenCalledWith(expect.objectContaining({ lessonType: 'positive', evidenceSources: ['manual-analytics'], diagnosis: expect.stringContaining('not a causal claim') }), 'active');
  ...
  it('promotes a negative candidate with failure dimensions', async () => {
    ...
    const narrative = { _id: 'n1', personaId: 'active', topic: 'observasi', linkPlacement: 'middle' };
    const narratives = { findById: jest.fn(() => ({ lean: jest.fn().mockResolvedValue(narrative) })), find: jest.fn(() => ({ lean: jest.fn().mockResolvedValue([narrative]) })), updateOne: jest.fn().mockResolvedValue({}) };
    ...
    expect(createLesson).toHaveBeenCalledWith(expect.objectContaining({ lessonType: 'negative', failureDimensions: ['narrative', 'reference'], recommendedFix: 'Cari jembatan referensi yang lebih jelas.' }), 'active');
  });

  it('rejects promotion when no manual candidate exists', async () => {
    const analytics = analyticsRows([]);
    const narrative = { _id: 'n1', personaId: 'active', topic: 'observasi', linkPlacement: 'ending' };
  ...
  it('returns the existing lesson for repeated promotion', async () => {
    const narrative = { _id: 'n1', personaId: 'active', outcomeKnowledgeId: 'k1' };
