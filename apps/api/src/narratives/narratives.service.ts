...
Fix every reviewer failure. Open with the persona's first-person voice and a concrete information gap. Show the narrator thinking or doubting before the conclusion. Keep every activity, object, and environment in one believable scene unless an explicit comparison bridge is present. Replace abstract metaphors and generic adjectives with a specific sensory observation. Make the title conversational, not journalistic. Follow at least one real concern before introducing the reference; remove shopping transitions, marketplace descriptions, unsupported community conclusions, and product-led closing questions. If there is a URL, name its reference context before the URL and make the connection explicit.`,
        maxTokens: 900,
        json: true,
      });
