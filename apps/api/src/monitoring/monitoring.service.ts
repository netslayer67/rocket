...
import { analyticsEvent, feedbackEvent, jobEvent, knowledgeEvent, learningEvent, modelEvent, MonitoringEvent, MonitoringSnapshot } from './monitoring-events';
...
export class MonitoringService {
  ...
  async history(): Promise<MonitoringSnapshot> {
    ...
    const [jobs, runs, feedback, logs, knowledge, analytics] = await Promise.all([
    ...
    const events = [...jobs.map(jobEvent), ...runs.map(modelEvent), ...feedback.map(feedbackEvent), ...logs.map(learningEvent), ...knowledge.map(knowledgeEvent), ...analytics.map(analyticsEvent)]
      .sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).slice(0, 100);
    return snapshot(windowStart, events);
  }
    ...
    return new Observable((subscriber) => {
      ...
      const poll = async () => {
        ...
        try {
          ...
          const signature = current.events.map((item) => `${item.id}:${item.status}:${item.occurredAt}`).join('|');
